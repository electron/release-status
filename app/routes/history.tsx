...
export const loader = async (args: LoaderFunctionArgs) => {
  ...
  const releasesThisYear = releases.filter((r) => r.date.split('-')[0] === `${year}`);
  ...
  for (const release of releasesThisYear) {
    const month = months[parseInt(release.date.split('-')[1], 10) - 1];
    const day = parseInt(release.date.split('-')[2], 10);
...
export default function ReleaseHistory() {
  ...
  const { data: calendarData, year } = useLoaderData<typeof loader>();
